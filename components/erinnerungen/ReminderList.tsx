...
export default function ReminderList({ reminders, role, userId, classId, viewersByReminder = {}, allStudentNames = [], myViewedIds = [] }: Props) {
  const router = useRouter()
  const [showModal, setShowModal] = useState(false)
  const [viewedSet, setViewedSet] = useState(new Set(myViewedIds))

  const today = new Date(); today.setHours(0,0,0,0)
  const upcoming = reminders.filter(r => new Date(r.event_date) >= today)
  const past = reminders.filter(r => new Date(r.event_date) < today)
  ...
  return (
    <>
      ...
      {showModal && (
        <AddReminderModal classId={classId} userId={userId} onClose={() => setShowModal(false)} />
      )}
    </>
  )
}
...
function ReminderCard({
  reminder: r, role, userId, onDelete, onMarkSeen, viewedByMe, viewers, allStudentNames,
}: {
  reminder: Reminder
  role: Role
  userId: string
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm flex gap-4 items-start group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Teacher: viewer info */}
        {role === 'teacher' && allStudentNames.length > 0 && (
