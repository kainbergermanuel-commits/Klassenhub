        ...
        return (
          <div key={arc.name} className="relative overflow-hidden rounded-2xl kh-card p-5">
            ...
            {locationArt ? (
              ...
              // Text sitzt ohnehin auf eigenen Glas-Panels und bleibt lesbar.
            ...
            {/* ── Inhalt: Figur links, Text rechts ────────────────────────── */}
            <div className="relative z-10 flex gap-4">
              {/* Volle Guide-Figur (nicht rundes Portrait) im linken Bereich.
                  sticky, damit sie bei langen Etappenlisten mitwandert. */}
              {!locationArt && portrait && (
                <div className="w-[120px] md:w-[150px] flex-shrink-0 self-stretch">
                  <img
                    src={portrait}
                    alt={arc.guide}
                    className="sticky top-4 w-full max-h-[75vh] object-contain object-bottom drop-shadow-[0_8px_16px_rgba(20,40,45,.35)]"
                  />
                </div>
              )}
              ...
              <div className="min-w-0 flex-1">
                {/* Kopf-Panel (Glas), damit der Text über dem Bild lesbar bleibt */}
                <div className="rounded-xl bg-white/75 backdrop-blur-sm px-3.5 py-2.5 shadow-sm">
                  ...
                  <p className="text-[11.5px] text-kh-dark/70 leading-snug mt-1.5">
                ...
                {story ? (
                  <div className="mt-3 flex flex-col gap-2.5">
                    <p className="text-[11px] font-bold uppercase tracking-wide text-kh-dark/70 bg-white/60 rounded-md px-2 py-1 inline-block w-fit">
                    ...
                    {story.stages.map((stage, i) => (
                      <div key={stage.label} className="rounded-xl bg-white/85 backdrop-blur-sm p-3 shadow-sm">
                    ...
                    {builtTheme && (
                      <p className="text-[12px] text-kh-dark/70 italic bg-white/60 rounded-md px-2 py-1 w-fit">Nudge-Beispiel (3 offen): {builtTheme.nudge(3)}</p>
                    )}
                  </div>
                ) : (
                  <p className="mt-3 text-[12.5px] text-kh-dark/75 font-medium italic bg-white/70 rounded-md px-2.5 py-1.5 w-fit">
