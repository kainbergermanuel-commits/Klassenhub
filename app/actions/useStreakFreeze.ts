...
export async function useStreakFreeze(): Promise<{ newStreak: number }> {
  ...
  const [{ data: allHw }, { data: completions }, { data: freezes }] = await Promise.all([
  ...
  const breakingHwId = findBreakingHomework(doneIds, hw, today, frozenIds)
  ...
  const newStreak = computeStreak(doneIds, hw, today, new Set([...frozenIds, breakingHwId]))
