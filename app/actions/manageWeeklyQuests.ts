...
export async function swapWeeklyQuest(weekStart: string, oldKey: string, newKey: string): Promise<void> {
  ...
  if (!profile || profile.role !== 'teacher' || !profile.class_id) throw new Error('Unauthorized')
  if (!findQuestTemplate(newKey)) throw new Error('Unbekannte Quest-Vorlage')

  const supabase = await createClient()
  const { data: existing } = await supabase
    .from('quests').select('template_key').eq('class_id', profile.class_id).eq('week_start', weekStart)
  const currentKeys = (existing ?? []).map(q => q.template_key)
  const baseKeys = currentKeys.length > 0 ? currentKeys : defaultWeeklyTemplateKeys(profile.class_id, weekStart)
  const nextKeys = baseKeys.map(k => (k === oldKey ? newKey : k))

  await supabase.from('quests').delete().eq('class_id', profile.class_id).eq('week_start', weekStart)
  if (nextKeys.length > 0) {
    const { error } = await supabase.from('quests').insert(
      nextKeys.map(k => ({ class_id: profile.class_id!, template_key: k, week_start: weekStart, created_by: profile.id })) as never
    )
...
export async function resetWeeklyQuests(weekStart: string): Promise<void> {
  ...
  const supabase = await createClient()
  const { error } = await supabase.from('quests').delete().eq('class_id', profile.class_id).eq('week_start', weekStart)
  if (error) throw new Error(error.message)
